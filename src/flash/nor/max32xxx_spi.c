...
static int max32xxx_spi_mass_erase(struct flash_bank *bank);
static int max32xxx_spi_setup_spim(struct flash_bank *bank);
static int max32xxx_spi_setup_spix(struct flash_bank *bank);
static int max32xxx_spi_exit_qpi(struct flash_bank *bank);
static int max32xxx_spi_probe(struct flash_bank *bank);

struct max32xxx_spi_flash_bank {
	int probed;
...
static uint8_t max32xxx_spi_write_code[] = {
...
/* Config Command: flash bank name driver base size chip_width bus_width target [driver_options]
		flash bank max32xxx_spi <base> <size> 0 0 <target> <flash_block> <flash_dummy> <flash_read>
 ...
 */
FLASH_BANK_COMMAND_HANDLER(max32xxx_spi_flash_bank_command)
{
	struct max32xxx_spi_flash_bank *maxim_info;
	...
	maxim_info = calloc(sizeof(struct max32xxx_spi_flash_bank), 1);
...
{
	int printed;
	struct max32xxx_spi_flash_bank *maxim_info = bank->driver_priv;
...
static int max32xxx_spi_setup_spim(struct flash_bank *bank)
{
	...
	struct max32xxx_spi_flash_bank *max32xxx_spi_info = bank->driver_priv;
	int retval;
	uint32_t tempvalue;
	...
	if (max32xxx_spi_info->probed == 0) {
		LOG_ERROR("Target not probed");
		return ERROR_FLASH_BANK_NOT_PROBED;
	}

	if ((max32xxx_spi_info->spim_io != 0) && (max32xxx_spi_info->spim_clk != 0)) {

		/* Disable I/O and clocking for SPIX */
		target_write_u32(target, max32xxx_spi_info->spix_io, 0);
		target_write_u32(target, max32xxx_spi_info->spix_clk, 0);

		/* Enable the I/O for the SPIM */
		target_write_u32(target, max32xxx_spi_info->spim_io, SPIM_IO);

		/* Check the I/O response */
		retval = target_read_u32(target, max32xxx_spi_info->spim_io + 0x4, &tempvalue);
		if (retval != ERROR_OK) {
		...
		target_write_u32(target, max32xxx_spi_info->spim_clk, SPIM_CLK);

	}

	/* Initialize the SPIM */
	target_write_u32(target, (max32xxx_spi_info->spim_base + SPIM_GEN_CTRL), SPIM_GEN_CTRL_INIT);
	target_write_u32(target, (max32xxx_spi_info->spim_base + SPIM_CFG), SPIM_CFG_INIT);

	max32xxx_spi_exit_qpi(bank);
...
static int max32xxx_spi_setup_spix(struct flash_bank *bank)
{
	...
	if ((max32xxx_spi_info->spix_io != 0) && (max32xxx_spi_info->spix_clk != 0)) {

		/* Disable I/O and clocking for SPIM */
		target_write_u32(target, max32xxx_spi_info->spim_io, 0);
		target_write_u32(target, max32xxx_spi_info->spim_clk, 0);

		/* Enable the I/O for the SPIX */
		target_write_u32(target, max32xxx_spi_info->spix_io, SPIX_IO);

		/* Check the I/O response */
		retval = target_read_u32(target, (max32xxx_spi_info->spix_io + 0x4), &tempvalue);
		if (retval != ERROR_OK) {
			LOG_WARNING("target_read_u32() failed with %d", retval);
			return retval;
		...
		target_write_u32(target, max32xxx_spi_info->spix_clk, SPIX_CLK);
	}

	/* Disable the SPIM */
	target_write_u32(target, (max32xxx_spi_info->spim_base + SPIM_GEN_CTRL), 0);

	/* Initialize the SPIX */
	target_write_u32(target, max32xxx_spi_info->spix_base + SPIX_CFG, SPIX_CFG_INIT);
	target_write_u32(target, max32xxx_spi_info->spix_base + SPIX_MODE,
		(SPIX_MODE_INIT | (max32xxx_spi_info->flash_dummy & 0xF)));

	target_write_u32(target, max32xxx_spi_info->spix_base + SPIX_FETCH,
		(max32xxx_spi_info->flash_read & 0xFF));

	/* Enable or disable the decryption */
	if (max32xxx_spi_info->crypto_base != 0)
		target_write_u32(target, max32xxx_spi_info->spix_base + SPIX_CRYPTO, SPIX_CRYPTO_INIT);
	else
		target_write_u32(target, max32xxx_spi_info->spix_base + SPIX_CRYPTO, 0x0);

	return ERROR_OK;
}

static int max32xxx_spi_protect_check(struct flash_bank *bank)
{
	struct max32xxx_spi_flash_bank *maxim_info = bank->driver_priv;
	int i;
...
static int max32xxx_spi_busy(struct flash_bank *bank)
{
	struct target *target = bank->target;
	struct max32xxx_spi_flash_bank *max32xxx_spi_info = bank->driver_priv;
	int retval;
	...
	target_write_u16(target, max32xxx_spi_info->spim_txfifo, header);
	target_write_u16(target, max32xxx_spi_info->spim_txfifo, CMD_SR);

	/*             read  | bytes      | 1 byte     | deassert SS */
	header = ((0x1 << 1) | (0x1 << 2) | (0x1 << 4) | (0x1 << 13));

	/* Clear the Results done flag */
	target_write_u32(target, max32xxx_spi_info->spim_base + SPIM_INTFL,
		SPIM_INTFL_RES_DONE);
	...
	timeout = TIMEOUT_CNT;
	while (1) {
		retval = target_read_u32(target, max32xxx_spi_info->spim_base + SPIM_INTFL, &temp32);
		if (retval != ERROR_OK) {
			LOG_WARNING("target_read_u32() failed with %d", retval);
			return retval;
	...
	/* Read the results */
	retval = target_read_u8(target, max32xxx_spi_info->spim_rxfifo, &temp8);
	if (retval != ERROR_OK) {
...
static void max32xxx_spi_cmd(struct flash_bank *bank, uint32_t cmd, uint8_t deass)
{
	struct target *target = bank->target;
	struct max32xxx_spi_flash_bank *max32xxx_spi_info = bank->driver_priv;
	...
	target_write_u16(target, max32xxx_spi_info->spim_txfifo, header);
	target_write_u16(target, max32xxx_spi_info->spim_txfifo, CMD_WE);

	/* Send the command */
	/*               write | bytes      | 1 byte     | deassert SS */
	header = (0x1 | (0x1 << 2) | (0x1 << 4) | (deass << 13));
	target_write_u16(target, max32xxx_spi_info->spim_txfifo, header);
	target_write_u16(target, max32xxx_spi_info->spim_txfifo, cmd);
}
...
static int max32xxx_spi_exit_qpi(struct flash_bank *bank)
{
	...
	target_write_u16(target, max32xxx_spi_info->spim_txfifo, header);
	target_write_u16(target, max32xxx_spi_info->spim_txfifo, 0xFFFF);
	target_write_u16(target, max32xxx_spi_info->spim_txfifo, 0xFFFF);
	...
	target_write_u16(target, max32xxx_spi_info->spim_txfifo, header);
	target_write_u16(target, max32xxx_spi_info->spim_txfifo, CMD_EQPI0);
	...
	/* Send the EXIT QPI command */
	target_write_u16(target, max32xxx_spi_info->spim_txfifo, header);
	target_write_u16(target, max32xxx_spi_info->spim_txfifo, CMD_EQPI1);

	return ERROR_OK;
}

static int max32xxx_spi_erase(struct flash_bank *bank, int first, int last)
{
	struct max32xxx_spi_flash_bank *max32xxx_spi_info = bank->driver_priv;
	struct target *target = bank->target;
	int retval, banknr;
	...
	if (max32xxx_spi_info->integrity) {
		total_len = (last + 1) * max32xxx_spi_info->flash_block;
		total_len += ((total_len/0x80)+1)*0x10;
		last = (total_len / max32xxx_spi_info->flash_block) + 1;
	}

	if ((first == 0) && (last == (bank->num_sectors - 1)))
		return max32xxx_spi_mass_erase(bank);

	retval = max32xxx_spi_setup_spim(bank);
	if (retval != ERROR_OK) {
		LOG_ERROR("Failed to initialize SPIM peripheral");
		max32xxx_spi_setup_spix(bank);
		return retval;
	}
	...
	for (banknr = first; banknr <= last; banknr++) {
		...
		max32xxx_spi_cmd(bank, max32xxx_spi_info->flash_erase, 0);
		...
		uint16_t addr_temp = ((banknr * max32xxx_spi_info->flash_block >> 16) & 0xFF);
		addr_temp |= ((banknr * max32xxx_spi_info->flash_block) & 0xFF00);
		target_write_u16(target, max32xxx_spi_info->spim_txfifo, addr_temp);

		addr_temp = 0xF000;
		addr_temp |= ((banknr * max32xxx_spi_info->flash_block) & 0xFF);
		target_write_u16(target, max32xxx_spi_info->spim_txfifo, addr_temp);

		/* Wait for the flash to not be busy */
		int timeout = 0xFFF;
		do {
			retval = max32xxx_spi_busy(bank);
			if (timeout-- == 1) {
				LOG_ERROR("Timeout waiting for external flash");
				return retval;
			}
		} while (retval == ERROR_WAIT);
	...
	return max32xxx_spi_setup_spix(bank);
}

static int max32xxx_spi_protect(struct flash_bank *bank, int set, int first, int last)
{
	struct max32xxx_spi_flash_bank *maxim_info = bank->driver_priv;
...
static int max32xxx_spi_write_blob(struct flash_bank *bank, const uint8_t *buffer, uint32_t offset, uint32_t count)
{
	struct max32xxx_spi_flash_bank *max32xxx_spi_info = bank->driver_priv;
	struct target *target = bank->target;
	...
	/* Allocate the blob */
	if (target_alloc_working_area(target, sizeof(max32xxx_spi_write_code),
	...
	target_write_buffer(target, write_algorithm->address,
		sizeof(max32xxx_spi_write_code), (uint8_t *) max32xxx_spi_write_code);

	LOG_WARNING("Working area addr = 0x%x size = 0x%x", write_algorithm->address,
		(unsigned int)(sizeof(max32xxx_spi_write_code) + buffer_size));
	...
	buf_set_u32(mem_param[0].value, 0, 32, max32xxx_spi_info->spim_base);
	buf_set_u32(mem_param[1].value, 0, 32, max32xxx_spi_info->crypto_base);
	buf_set_u32(mem_param[2].value, 0, 32, max32xxx_spi_info->spim_txfifo);
	buf_set_u32(mem_param[3].value, 0, 32, max32xxx_spi_info->spim_rxfifo);
	buf_set_u32(mem_param[4].value, 0, 32, max32xxx_spi_info->integrity);
...
static int max32xxx_spi_write_block(struct flash_bank *bank, const uint8_t *buffer, uint32_t offset, uint32_t count)
{
	struct max32xxx_spi_flash_bank *max32xxx_spi_info = bank->driver_priv;
	struct target *target = bank->target;
	...
	max32xxx_spi_cmd(bank, max32xxx_spi_info->flash_write, 0);
	...
	while (count > 0) {
		...
		while (header_count) {
			...
			while (1) {
				...
				target_write_u32(target, max32xxx_spi_info->spim_base + SPIM_INTFL, SPIM_INTFL_TXAE);
			...
			if (header_count > 1) {
				target_write_u16(target, max32xxx_spi_info->spim_txfifo, ((buffer[num]) | (buffer[num+1] << 8)));
				header_count -= 2;
				num += 2;
			} else if (header_count) {
				target_write_u16(target, max32xxx_spi_info->spim_txfifo, (0xF000 | buffer[num]));
				header_count -= 1;
				num += 1;
			}
		}
...
static int max32xxx_spi_write(struct flash_bank *bank, const uint8_t *buffer, uint32_t offset, uint32_t count)
{
	struct max32xxx_spi_flash_bank *max32xxx_spi_info = bank->driver_priv;
	...
	if (count >= 0x100) {
		retval = max32xxx_spi_write_blob(bank, buffer, offset, count);
	...
	if (remaining) {
		retval = max32xxx_spi_write_block(bank, buffer, offset, remaining);
		if (retval != ERROR_OK) {
			max32xxx_spi_setup_spix(bank);
			return retval;
		}
	...
	while (blocks_remaining) {
		retval = max32xxx_spi_write_block(bank, buffer, offset, WRITE_BLOCK);
		if (retval != ERROR_OK) {
			max32xxx_spi_setup_spix(bank);
			return retval;
		}
	...
	if (count) {
		retval = max32xxx_spi_write_block(bank, buffer, offset, count);
		if (retval != ERROR_OK) {
			max32xxx_spi_setup_spix(bank);
			return retval;
		}
...
static int max32xxx_spi_mass_erase(struct flash_bank *bank)
{
	...
	if (retval == ERROR_OK) {
		...
		max32xxx_spi_cmd(bank, max32xxx_spi_info->flash_mass_erase, 1);
...
static int max32xxx_spi_read(struct flash_bank *bank, uint8_t *buffer, uint32_t offset, uint32_t count)
{
	int retval;

	retval = max32xxx_spi_setup_spix(bank);
	if (retval != ERROR_OK)
		return retval;
...
COMMAND_HANDLER(max32xxx_spi_handle_mass_erase_command)
{
	int i;

	if (CMD_ARGC < 1) {
		command_print(CMD_CTX, "max32xxx_spi mass_erase <bank>");
		return ERROR_OK;
	}
	...
	if (max32xxx_spi_mass_erase(bank) == ERROR_OK) {
		/* set all sectors as erased */
		for (i = 0; i < bank->num_sectors; i++)
			bank->sectors[i].is_erased = 1;

		command_print(CMD_CTX, "max32xxx_spi mass erase complete");
	} else
		command_print(CMD_CTX, "max32xxx_spi mass erase failed");

	return ERROR_OK;
}

COMMAND_HANDLER(max32xxx_spi_handle_setup_spix_command)
{
	if (CMD_ARGC < 1) {
		command_print(CMD_CTX, "max32xxx_spi setup_spix <bank>");
		return ERROR_OK;
	}
	...
	if (max32xxx_spi_setup_spix(bank) == ERROR_OK)

		command_print(CMD_CTX, "max32xxx_spi setup_spix complete");
	else
		command_print(CMD_CTX, "max32xxx_spi setup_spix failed");

	return ERROR_OK;
}

COMMAND_HANDLER(max32xxx_spi_handle_setup_spim_command)
{
	if (CMD_ARGC < 1) {
		command_print(CMD_CTX, "max32xxx_spi setup_spim <bank>");
		return ERROR_OK;
	}
	...
	if (max32xxx_spi_setup_spim(bank) == ERROR_OK)

		command_print(CMD_CTX, "max32xxx_spi setup_spim complete");
	else
		command_print(CMD_CTX, "max32xxx_spi setup_spim failed");

	return ERROR_OK;
}

static const struct command_registration max32xxx_spi_exec_command_handlers[] = {
	{
		.name = "mass_erase",
		.handler = max32xxx_spi_handle_mass_erase_command,
		.mode = COMMAND_EXEC,
		.usage = "bank_id",
	...
	{
		.name = "setup_spix",
		.handler = max32xxx_spi_handle_setup_spix_command,
		.mode = COMMAND_EXEC,
		.usage = "bank_id",
	...
	{
		.name = "setup_spim",
		.handler = max32xxx_spi_handle_setup_spim_command,
		.mode = COMMAND_EXEC,
		.usage = "bank_id",
...
static const struct command_registration max32xxx_spi_command_handlers[] = {
	{
		.name = "max32xxx_spi",
		.mode = COMMAND_EXEC,
		.help = "Maxim XIP flash command group",
		.chain = max32xxx_spi_exec_command_handlers,
	},
	COMMAND_REGISTRATION_DONE
};

struct flash_driver max32xxx_spi_flash = {
	.name = "max32xxx_spi",
	.commands = max32xxx_spi_command_handlers,
	.flash_bank_command = max32xxx_spi_flash_bank_command,
	.erase = max32xxx_spi_erase,
	.protect = max32xxx_spi_protect,
	.write = max32xxx_spi_write,
	.read = max32xxx_spi_read,
	.probe = max32xxx_spi_probe,
	.auto_probe = max32xxx_spi_probe,
	.erase_check = default_flash_blank_check,
	.protect_check = max32xxx_spi_protect_check,
	.info = get_maxim_info,
};
